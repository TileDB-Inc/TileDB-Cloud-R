...
ApiClient  <- R6::R6Class(
  ...
  public = list(
      ...
      if (!is.null(self$retryStatusCodes)) {
        ...
        for (i in 1 : self$maxRetryAttempts) {
          if (statusCode %in% self$retryStatusCodes) {
            ...
            print("CALLAPI PRE EXECUTE")
            resp <- self$Execute(url, method, queryParams, headerParams, body, ...)
            # https://www.rdocumentation.org/packages/httr/versions/1.4.2/topics/response
            print("CALLAPI RESP:B")
            #print(resp)
    ...
    deserialize = function(resp, returnType, pkgEnv) {
      print("DESERIALIZE ENTER")
      print("DESERIALIZE resp$content")
      print(class(resp$content))
      print("DESERIALIZE resp$content")
      print(typeof(resp$content))
      print("DESERIALIZE resp$content")
      print(resp$content)
      respObj <- jsonlite::fromJSON(httr::content(resp, "text", encoding = "UTF-8"))
      ##respObj <- unserialize(as.raw(jsonlite::fromJSON(httr::content(resp))))
