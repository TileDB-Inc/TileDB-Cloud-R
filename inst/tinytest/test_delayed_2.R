...
d <- delayed(function(x,y){x+y},   args=list(b,c), display_name='d')

dag <- new("DAG", namespace="ns", terminal_node=d$node)

expect_equal(length(a$node$parent_nodes), 0)
expect_equal(length(b$node$parent_nodes), 1)
expect_equal(length(c$node$parent_nodes), 1)
expect_equal(length(d$node$parent_nodes), 2)
